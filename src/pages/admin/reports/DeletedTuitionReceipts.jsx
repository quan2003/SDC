...
import { FiFileText, FiFilter, FiDownload, FiTrash2, FiRotateCcw, FiAlertCircle } from 'react-icons/fi';
import { useToast } from '../../../contexts/ToastContext';
import { formatCurrency, formatDateTime } from '../../../utils/helpers';

const mockDeleted = [
  { id: 1, receiptNo: 'PT-2026-001', studentName: 'Nguyễn Thị Mai', amount: 350000, deletedAt: '2026-04-10 09:30:00', deletedBy: 'admin', reason: 'Nhập nhầm thông tin học viên' },
  { id: 2, receiptNo: 'PT-2026-005', studentName: 'Hoàng Thị Linh', amount: 400000, deletedAt: '2026-04-11 14:15:00', deletedBy: 'staff1', reason: 'Học viên đăng ký sai chứng chỉ' },
  { id: 3, receiptNo: 'PT-2026-008', studentName: 'Trần Văn Hùng', amount: 350000, deletedAt: '2026-04-12 10:00:00', deletedBy: 'admin', reason: 'Test hệ thống' },
];

export default function DeletedTuitionReceipts() {
  ...
  return (
    <div className="animate-fade-in-up">
      <div className="page-header">
        <h1 className="page-title"><FiTrash2 /> Phiếu thu học phí đã xóa</h1>
      ...
      <div style={{ background: 'rgba(239, 68, 68, 0.08)', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: 'var(--radius-md)', padding: '12px 16px', display: 'flex', gap: 10, alignItems: 'center', marginBottom: 20, color: 'var(--danger-400)' }}>
        ...
        <span style={{ fontSize: '0.88rem' }}>Danh sách phiếu thu học phí đã bị xóa khỏi hệ thống. Dữ liệu chỉ dùng để kiểm tra và đối chiếu.</span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          ...
          { label: 'Tổng giá trị bị xóa', value: formatCurrency(records.reduce((s, r) => s + r.amount, 0)), color: 'var(--warning-400)' },
      ...
      <div className="toolbar">
        ...
        <div className="toolbar-right">
          <span style={{ fontSize: '0.85rem', color: 'var(--text-tertiary)' }}>
            Hiển thị <strong style={{ color: 'var(--text-primary)' }}>{filtered.length}</strong> / {records.length} phiếu
          </span>
        </div>
      </div>
      ...
      <div className="card">
        <table className="data-table">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: 40, color: 'var(--text-tertiary)' }}>Không có phiếu thu nào bị xóa trong giai đoạn này</td></tr>
            ) : filtered.map((r, i) => (
              <tr key={r.id}>
